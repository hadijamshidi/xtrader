...
$(function () {
    ...
    // Highcharts.theme.tooltip.footerFormat += '<small>'+this.x+'</small>';
    // console.log(Highcharts.theme.tooltip.footerFormat);
    // Apply the theme
    Highcharts.setOptions(Highcharts.theme);
    // waiting('wait');
    // $.getJSON('/get-data/name=' + symbol_name, function (data) {
...
function check_strategies_number() {
    ...
    document.getElementById('strategy place').setAttribute('style', 'main:none');
    if (stra_num > 0) {
        document.getElementById('strategy place').setAttribute('style', 'main:block');
        ...
        var txt = document.createTextNode('Back Test');
        ...
        var txt = document.createTextNode('Save Strategy');
        ...
        var txt = document.createTextNode('Scan Market');
        ...
        delete_all_button.setAttribute('style', 'float:right');
        ...
        delete_all_button.addEventListener('click', function () {
            delete_all(['indicators', 'back test', 'scan','filters'],true);
        });
...
function show_market() {
    ...
    div.setAttribute('style', 'main: block');
}
function delete_all(targets,save) {
    targets.forEach(function (target) {
        switch (target) {
            ...
            case 'scan':
                var div = document.getElementById('scan place');
                div.setAttribute('style', 'main: none');
                save_filters('default');
                isStrategySaved = false;
                break;
            default:
...
function apply() {
    ...
    var dd = {'name': symbol_name, 'trades': JSON.stringify(result2)};
    waiting('wait');
    ...
    if (num_of_trades != 0) {
        ...
        for (var i = num_of_trades; i > 0; i -= 1) {
            if (Object.keys(res["" + i]).length == 2) {
                tbl = [num_of_trades - i + 1, dohlcv[res["" + i]["sell"]["date"]][0], res["" + i]["sell"]['action'], dohlcv[res["" + i]["sell"]["date"]][4], 'NaN', res["" + i]["sell"]["candles in trade"], res["" + i]["sell"]["return"], res["" + i]["sell"]["capital"]];
            ...
            if (i == 1) {
                tbl = [num_of_trades - i + 1, dohlcv[res["" + i]["buy"]["date"]][0], res["" + i]["buy"]['action'], dohlcv[res["" + i]["buy"]["date"]][4], res["" + i]["buy"]["waiting candles"], 'NaN', 'NaN', 'Initial Deposit :' + res["" + i]["buy"]["initaial deposit"]];
                buy_b.unshift([dohlcv[res["" + i]["buy"]["date"]][0], dohlcv[res["" + i]["buy"]["date"]][4]]);
                appendRow(tbl);
            } else {
                tbl = [num_of_trades - i + 1, dohlcv[res["" + i]["buy"]["date"]][0], res["" + i]["buy"]['action'], dohlcv[res["" + i]["buy"]["date"]][4], res["" + i]["buy"]["waiting candles"], 'NaN', 'NaN', 'NaN'];
                buy_b.unshift([dohlcv[res["" + i]["buy"]["date"]][0], dohlcv[res["" + i]["buy"]["date"]][4]]);
                appendRow(tbl);
            }
        }
        ...
        tbl = ['', '', '', 'Standard Deviation', std['waiting candles'], std['candles in trade'], String(std['profits']) + '%', "NaN"];
        appendRow(tbl);
...
function save_filters(pointer) {
    ...
    var strategy = {'name': 'my strategy', 'filters': filters};
...
function apply_strategy(name) {
    ...
    $.ajax({
        type: 'POST',
        url: "/fin/get_strategy_filters",
        data: {
            param: name,
            csrfmiddlewaretoken: $('input[name=csrfmiddlewaretoken]').val()
        },
        success: function (filters) {
            var strategy = JSON.parse(filters);
...
function add_table() {
    ...
    var t = document.createTextNode("Trade num");
    z.appendChild(t);
    ...
    var z = document.createElement("TH");
    var t = document.createTextNode("Trade Type");
    z.appendChild(t);
    document.getElementById("myTr").appendChild(z);
    var z = document.createElement("TH");
    var t = document.createTextNode("Price: Close");
    z.appendChild(t);
    document.getElementById("myTr").appendChild(z);

    var z = document.createElement("TH");
    var t = document.createTextNode("Waiting Candles");
    z.appendChild(t);
    document.getElementById("myTr").appendChild(z);


    var z = document.createElement("TH");
    var t = document.createTextNode("Candles in Trade");
    z.appendChild(t);
    document.getElementById("myTr").appendChild(z);

    var z = document.createElement("TH");
    var t = document.createTextNode("Profit");
    z.appendChild(t);
    document.getElementById("myTr").appendChild(z);

    var z = document.createElement("TH");
    var t = document.createTextNode("Deposit");
    z.appendChild(t);
    document.getElementById("myTr").appendChild(z);
}
...
function insert_indicators() {
    ...
    // var idss = ['candlestick'];
    // for (var ids in idss) {
    //     var place = document.getElementById(idss[ids]);
    //     var group = document.createElement('optgroup');
    //     group.setAttribute('label', 'candlestick patterns');
    //     place.appendChild(group);
    //
    //     Object.keys(indicators['Pattern Recognition']).forEach(function (indicator) {
    //         indic[indicator] = indicators['Pattern Recognition'][indicator];
    //
    //         var ind = document.createElement('option');
    //         ind.setAttribute('value', indicator);
    //         t = document.createTextNode(indicator);
...
function show_div(id) {
    var idss = ["draw line", "just draw", "Technical_Patterns", "cross", "ascending_main", "more_than", "special_methods", "advance_cross"];
...
function describe(meth) {
    var desc = {
        ...
        'MACD(Moving Average Convergence/Divergence)': 'Buy when macd is above signal line and macd crosses zero line upward and sell when macd is below signal line and crosses zero line downward',
    };
...
function add(data) {
    ...
    switch (data['kind']) {
        ...
        case 'ascending':
            ...
            title = brief['abr'] + ' being ascending';
            brief = brief['abr'] + ' be ascending';
            break;
        case 'more':
            var brief1 = give_abr({
        ...
        case 'special':
            ...
            title = brief['abr'] + ' special method';
            brief = brief['abr'] + ' special method';
            break;
        case 'draw':
            brief = give_abr({
            ...
            title = 'draw ' + brief['abr'];
            brief = 'draw ' + brief['abr'];
            break;
        case 'candlestick':
            brief = give_abr({
...
function insert_strategy(data) {
    ...
    ic.setAttribute('class', 'icon checkmark');
    ic.setAttribute('style', 'text-align: left; float: left;cursor:pointer;');
    ic.setAttribute('title', 'delete');
...
$(document).ready(function () {
    $('input.prompt').attr('style', 'background-color: #333;color:white;text-align: center;');
